...
function StatefulSets({ namespace }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/kubernetes/statefulsets?namespace=${namespace}`);
  ...
  return (
    <Box className="fade-in">
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">StatefulSets</Typography>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead><TableRow>
            ...
            <TableCell>Ready</TableCell><TableCell>Age</TableCell>
          </TableRow></TableHead>
          <TableBody>
            {filtered.map((item) => (
              <TableRow key={`${item.metadata.namespace}-${item.metadata.name}`}>
                ...
                <TableCell>{`${item.status.readyReplicas || 0}/${item.spec.replicas || 0}`}</TableCell>
                <TableCell>{formatDistanceToNow(new Date(item.metadata.creationTimestamp), { addSuffix: true })}</TableCell>
              </TableRow>
            ))}
            {filtered.length === 0 && <TableRow><TableCell colSpan={5} align="center" sx={{ py: 4 }}>
              <Typography variant="body2" sx={{ color: '#8b949e' }}>No statefulsets found</Typography>
            </TableCell></TableRow>}
          </TableBody>
        </Table>
      </TableContainer>
